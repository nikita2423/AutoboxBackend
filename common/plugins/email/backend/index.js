...
module.exports = function(server, databaseObj, helper, packageObj) {
	...
	var addRestMethod = function(modelConfigData){
        var cloneModelConfigDataObj = clone(modelConfigData);
		var emailModelName = cloneModelConfigDataObj.emailModel;
		...
		modelObj.getMailSchema = function(callback) {
            if(cloneModelConfigDataObj.templates){
                for (var key in cloneModelConfigDataObj.templates) {
                    if(cloneModelConfigDataObj.templates.hasOwnProperty(key)){
                        var template = cloneModelConfigDataObj.templates[key];
                        if(template){
                            if(!template.display){
                                //Remove the template from modelConfig data..
                                delete cloneModelConfigDataObj.templates[key];
                            }
                        }
                    }
                }
            }
            callback(null, cloneModelConfigDataObj);
		};
