...
public class SnaphyAclDb{
  ...
  public SnaphyAclDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<SnaphyAcl>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<SnaphyAcl> modelList = new DataList<SnaphyAcl>();
    ...
    // Getting All Data where
    public DataList<SnaphyAcl>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public void update__db(final String id,   final SnaphyAcl _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`SnaphyAcl`", values, "id = ?", new String[] { id });
    }
