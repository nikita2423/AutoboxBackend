...
public class ImportDataDb{
  ...
  public ImportDataDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<ImportData>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<ImportData> modelList = new DataList<ImportData>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM ImportData " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM ImportData " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<ImportData>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `ImportData` " + whereQuery + " LIMIT " + limit;
            }else{
                countQuery = "SELECT  * FROM `ImportData` " + whereQuery;
            }
        }
    ...
    public void update__db(final String id,   final ImportData _modelData) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                ContentValues values = getContentValues(_modelData);
                // updating row
                db.update("`ImportData`", values, "id = ?",
                        new String[] { id });
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
