...
public class SoldViaAutoboxDb{
  ...
  public SoldViaAutoboxDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    //super(context, DATABASE_NAME, null, DATABASE_VERSION);
    this.context = context;
    this.restAdapter = restAdapter;
    TABLE = "SoldViaAutobox";
    ...
    public void insert__db (final String id, final SoldViaAutobox _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      // Inserting Row
      ContentValues values = getContentValues(_modelData);
      db.insert("`SoldViaAutobox`", null, values);
    }
    ...
    public ContentValues getContentValues(SoldViaAutobox _modelData){
    ...
    // Getting single c
    public   SoldViaAutobox get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("SoldViaAutobox", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    if (hashMap != null) {
                        SoldViaAutoboxRepository repo = restAdapter.createRepository(SoldViaAutoboxRepository.class);
                        repo.addStorage(context);
                        return (SoldViaAutobox)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    // Getting single cont
    public   SoldViaAutobox get__db(String whereKey, String whereKeyValue) {
        if (whereKeyValue != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("`SoldViaAutobox`", null, "`" + whereKey + "` =?", new String[]{whereKeyValue}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    return null;
                }else{
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(4) != null){
                          vehicleInfoIdData = cursor.getString(4);
                        ...
                        if(cursor.getString(7) != null){
                          customerQuoteIdData = cursor.getString(7);
    ...
    public void upsert__db(String id, SoldViaAutobox model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<SoldViaAutobox>  getAll__db() {
        DataList<SoldViaAutobox> modelList = new DataList<SoldViaAutobox>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `SoldViaAutobox`";

        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
        //http://www.tothenew.com/blog/sqlite-locking-and-transaction-handling-in-android/
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<SoldViaAutobox>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    SoldViaAutoboxRepository repo = restAdapter.createRepository(SoldViaAutoboxRepository.class);
                    repo.addStorage(context);
                    modelList.add((SoldViaAutobox)repo.createObject(hashMap));
                }
    ...
    public DataList<SoldViaAutobox>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    public DataList<SoldViaAutobox>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<SoldViaAutobox> modelList = new DataList<SoldViaAutobox>();
        String whereQuery = getWhereQuery(whereKeyValue);
        String selectQuery;
        if(orderBy != null){
            selectQuery = "SELECT  * FROM `SoldViaAutobox` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM SoldViaAutobox " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                selectQuery = "SELECT  * FROM SoldViaAutobox " + whereQuery  + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    // Getting All Data where
    public DataList<SoldViaAutobox>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        String countQuery;
        if(orderBy != null){
            countQuery = "SELECT  * FROM `SoldViaAutobox` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `SoldViaAutobox` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `SoldViaAutobox` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void checkOldData__db(final HashMap<String, Object> whereKeyValue) {
            ...
            // updating row
            db.update("`SoldViaAutobox`", values, "_DATA_UPDATED = 1 AND " + where, null);
    }
    ...
    public void deleteOldData__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`SoldViaAutobox`", "_DATA_UPDATED = 0 AND " + where , null);
    }
    ...
    public void delete__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`SoldViaAutobox`", where , null);
    }
    ...
    // Getting All Data where
    public DataList<SoldViaAutobox>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<SoldViaAutobox> modelList = new DataList<SoldViaAutobox>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `SoldViaAutobox` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
      String countQuery = "SELECT  * FROM `SoldViaAutobox` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
      Cursor cursor = db.rawQuery(countQuery, null);
      int count = cursor.getCount();
      cursor.close();
    ...
    public void checkOldData__db(final String whereKey, final String whereKeyValue) {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`SoldViaAutobox`", values, "_DATA_UPDATED = 1 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }
    ...
    public void deleteOldData__db(final String whereKey, final String whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`SoldViaAutobox`", "_DATA_UPDATED = 0 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }


    //Update multiple data at once..
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final SoldViaAutobox _modelData ){
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      String where = getWhere(whereKeyValue);
      db.update("`SoldViaAutobox`", values, where, null);
    }
    ...
    public void update__db(final String id,   final SoldViaAutobox _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`SoldViaAutobox`", values, "id = ?", new String[] { id });
    }
    ...
    public void checkOldData__db() {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`SoldViaAutobox`", values, "_DATA_UPDATED = 1", null);
    }
    ...
    public void deleteOldData__db() {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`SoldViaAutobox`", "_DATA_UPDATED = 0", null);
    }
