...
public class SoldViaAutoboxDb{
  ...
  public SoldViaAutoboxDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<SoldViaAutobox>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<SoldViaAutobox> modelList = new DataList<SoldViaAutobox>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM SoldViaAutobox " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM SoldViaAutobox " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<SoldViaAutobox>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `SoldViaAutobox` " + whereQuery + " LIMIT " + limit;
            }else{
                countQuery = "SELECT  * FROM `SoldViaAutobox` " + whereQuery;
            }
        }
    ...
    public void update__db(final String id,   final SoldViaAutobox _modelData) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                ContentValues values = getContentValues(_modelData);
                // updating row
                db.update("`SoldViaAutobox`", values, "id = ?",
                        new String[] { id });
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
