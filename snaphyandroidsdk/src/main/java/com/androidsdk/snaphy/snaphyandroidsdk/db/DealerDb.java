...
public class DealerDb{
  ...
  public DealerDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<Dealer>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<Dealer> modelList = new DataList<Dealer>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM Dealer " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                selectQuery = "SELECT  * FROM Dealer " + whereQuery  + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    // Getting All Data where
    public DataList<Dealer>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `Dealer` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `Dealer` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void update__db(final String id,   final Dealer _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`Dealer`", values, "id = ?", new String[] { id });
    }
