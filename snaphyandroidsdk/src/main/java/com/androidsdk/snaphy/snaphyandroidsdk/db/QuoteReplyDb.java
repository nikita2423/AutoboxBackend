...
public class QuoteReplyDb{
  ...
  public QuoteReplyDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<QuoteReply>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<QuoteReply> modelList = new DataList<QuoteReply>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM QuoteReply " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                selectQuery = "SELECT  * FROM QuoteReply " + whereQuery  + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    // Getting All Data where
    public DataList<QuoteReply>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `QuoteReply` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `QuoteReply` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void update__db(final String id,   final QuoteReply _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`QuoteReply`", values, "id = ?", new String[] { id });
    }
