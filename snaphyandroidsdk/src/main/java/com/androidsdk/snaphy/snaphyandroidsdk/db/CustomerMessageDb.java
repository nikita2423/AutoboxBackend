...
public class CustomerMessageDb{
  ...
  public CustomerMessageDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<CustomerMessage>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<CustomerMessage> modelList = new DataList<CustomerMessage>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM CustomerMessage " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                selectQuery = "SELECT  * FROM CustomerMessage " + whereQuery  + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    // Getting All Data where
    public DataList<CustomerMessage>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `CustomerMessage` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `CustomerMessage` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void update__db(final String id,   final CustomerMessage _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`CustomerMessage`", values, "id = ?", new String[] { id });
    }
