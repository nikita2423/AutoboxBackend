...
public class GpsPacketDataDb{
  ...
  public GpsPacketDataDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<GpsPacketData>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<GpsPacketData> modelList = new DataList<GpsPacketData>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM GpsPacketData " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                selectQuery = "SELECT  * FROM GpsPacketData " + whereQuery  + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    // Getting All Data where
    public DataList<GpsPacketData>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `GpsPacketData` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `GpsPacketData` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void update__db(final String id,   final GpsPacketData _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`GpsPacketData`", values, "id = ?", new String[] { id });
    }
